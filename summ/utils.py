...
def build_summary(top_scores, doc, sentences, max_len=280):
    ...
    # Trim summary
    post = Post()
    print('[Summary before post-processing]', flush=True)
    print(summary, flush=True)
    print('', flush=True)
    print('', flush=True)
    summary = post.rep_search(summary)
    print('[Keyword summary before post-processing]', flush=True)
    ...
    if len(summary) < summary_size:
        ...
        summary, keyword_summary = _build_summary_2(sentences, all_sents, sents_to_add)
        print('[Summary before 2nd post-processing]', flush=True)
        print(summary, flush=True)
        print('', flush=True)
        print('', flush=True)
        summary = post.rep_search(summary)
        print('[Keyword summary before 2nd post-processing]', flush=True)
        print(keyword_summary, flush=True)
        print('', flush=True)
        print('', flush=True)
        keyword_summary = post.rep_search(keyword_summary)
